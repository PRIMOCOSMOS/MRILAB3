...
function pri = load_segmentation_priors(anatSize, segTpl)
    ...
    try
        ...
        assert(numel(idx) == 3, ...
            'cfg.templates.segmentation.tpmVolumeIndices 长度必须为3；当前值=%s。', mat2str(idx));
        assert(all(mod(idx, 1) == 0), ...
            'cfg.templates.segmentation.tpmVolumeIndices 必须为整数；当前值=%s。', mat2str(idx));
        assert(all(idx >= 1), ...
            'cfg.templates.segmentation.tpmVolumeIndices 必须为正整数；当前值=%s。', mat2str(idx));
        assert(all(idx <= size(tpm, 4)), ...
            'cfg.templates.segmentation.tpmVolumeIndices 越界：当前值=%s，允许范围=[1, %d]。', ...
