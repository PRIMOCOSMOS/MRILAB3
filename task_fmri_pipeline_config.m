...
% -------------------------------------------------------------------------
    ...
    if ispc
        cfg.templates.installRoots = { ...
            'D:\spm', ...
            'D:\DPABI_V9.0_250415\DPARSF'}; % Windows 默认安装根目录；用途：开箱即用推断 SPM/DPABI 模板目录；影响：可自动命中官方模板文件。
    else
        cfg.templates.installRoots = {}; % 非 Windows 默认留空；用途：避免跨平台硬编码路径失效；影响：建议通过环境变量或手动路径覆盖。
    end
    cfg.templates.installRootEnvVars = {'SPM_DIR', 'DPABI_DIR', 'DPARSF_DIR'}; % 环境变量名；用途：在不同机器上无侵入覆盖 installRoots；影响：设置后优先参与模板目录推断。
